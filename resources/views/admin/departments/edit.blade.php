...
<style>
    ...
    .danger-zone-text { font-size: .8125rem; color: #b91c1c; }
    .danger-zone-text strong { display: block; margin-bottom: .125rem; font-size: .875rem; }
    .btn-danger {
...
@section('content')
    ...
    <p class="page-subtitle">Update the details for <strong>{{ $department->name }}</strong>.</p>
    ...
    <div class="form-card">
        ...
        <div class="form-card-body">
            ...
            <form method="POST" action="{{ route('admin.departments.update', $department) }}" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <div style="display:flex; align-items:center; gap:.625rem;
                        ...
                        <input type="checkbox" id="has_saturday_leave" name="has_saturday_leave" value="1"
                               {{ old('has_saturday_leave', $department->has_saturday_leave) ? 'checked' : '' }}
                ...
                <div class="form-row mb-4">
                    <div>
                        ...
                        <select id="saturday_rule" name="saturday_rule"
                            ...
                            <option value="none"                  {{ old('saturday_rule', $department->saturday_rule) === 'none'                  ? 'selected' : '' }}>No Rule</option>
                            <option value="2nd_saturday_off"      {{ old('saturday_rule', $department->saturday_rule) === '2nd_saturday_off'      ? 'selected' : '' }}>2nd Saturday Off</option>
                            <option value="all_saturdays_off"     {{ old('saturday_rule', $department->saturday_rule) === 'all_saturdays_off'     ? 'selected' : '' }}>All Saturdays Off</option>
                            <option value="alternating_saturdays" {{ old('saturday_rule', $department->saturday_rule) === 'alternating_saturdays' ? 'selected' : '' }}>Alternating Saturdays</option>
                        </select>
                        @error('saturday_rule')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
            ...
            <div class="danger-zone">
                ...
                <form method="POST"
                      ...
                      onsubmit="return confirm('Delete department «{{ $department->name }}»? This cannot be undone.')">
                    @csrf
                    @method('DELETE')
