...
<style>
    ...
    .ip-info {
        font-size: .72rem; color: var(--text-muted); margin-top: .75rem;
    }
    .location-pill {
    ...
    .cal-legend {
        ...
        margin-top: 1rem; font-size: .72rem; color: var(--text-secondary);
    }
    ...
    .holiday-date-box {
        ...
        border-radius: 6px; padding: .25rem .3rem; flex-shrink: 0;
    }
    ...
    /* Leave blocks (reused from before, now smaller) */
    .leave-block {
        background: var(--surface); border: 1px solid var(--border);
        border-radius: var(--radius-md); box-shadow: var(--shadow-sm);
        overflow: hidden; margin-bottom: 1rem;
    ...
    .leave-block-header h6 {
        font-size: .78rem; font-weight: 700; color: var(--text-main); margin: 0;
        text-transform: uppercase; letter-spacing: .04em;
    }
    .leave-block-header .blk-note {
        margin-left: auto; font-size: .68rem; color: var(--text-muted); white-space: nowrap;
    }
    ...
    .mini-stat {
        flex: 1; padding: .625rem .75rem; border-right: 1px solid var(--border); text-align: center;
    }
    ...
    .btn-modal-cancel  {
        ...
        background: #f1f5f9; border: 1px solid var(--border);
        font-size: .875rem; font-weight: 600; cursor: pointer; color: var(--text-main);
    }
    .btn-modal-confirm {
        flex: 1; padding: .6rem; border-radius: var(--radius-sm);
...
<p class="dash-sub">{{ now()->format('l, d F Y') }} &mdash; Your attendance &amp; leave overview.</p>
...
<div class="kpi-grid">
    ...
    <div class="kpi-card">
        ...
        <div>
            <div class="kpi-val">{{ number_format($kpi['leave_days'], 1) }}</div>
            <div class="kpi-lbl">Leave Days</div>
        </div>
    </div>
    <div class="kpi-card">
        ...
        <div>
            <div class="kpi-val">{{ number_format($kpi['perm_hours'], 0) }}h</div>
            <div class="kpi-lbl">Perm. Hours</div>
        </div>
    </div>
    <div class="kpi-card">
    ...
    <div>
        ...
        <div class="d-card">
            ...
            <div class="punch-wrap">
                ...
                <form method="POST" action="{{ route('employee.punch.in') }}" id="punchInForm">
                    @csrf
            ...
            <div class="punch-wrap">
                ...
                <form method="POST" action="{{ route('employee.punch.out') }}" id="punchOutForm">
                    @csrf
    ...
    <div>
        ...
        <div class="aside-card">
            ...
            <div class="aside-card-body">
                @forelse($upcomingHolidays as $holiday)
                    @php
                        $daysLeft = now()->startOfDay()->diffInDays($holiday->date->startOfDay(), false);
                    @endphp
        ...
        <div class="leave-block">
            ...
            <div class="leave-block-body">
                <div class="mini-stat-row">
                    <div class="mini-stat">
                        <div class="mini-stat-val val-credited">{{ number_format($leave['cl_credited'], 0) }}</div>
                        <div class="mini-stat-lbl">Credited</div>
                    </div>
                    <div class="mini-stat">
                        <div class="mini-stat-val val-used">{{ number_format($leave['cl_used'], 0) }}</div>
                        <div class="mini-stat-lbl">Used</div>
                    </div>
                    <div class="mini-stat">
                        <div class="mini-stat-val val-balance">{{ number_format($leave['cl_balance'], 0) }}</div>
        ...
        <div class="leave-block">
            ...
            <div class="leave-block-body">
                <div class="mini-stat-row">
                    <div class="mini-stat">
                        <div class="mini-stat-val val-credited">{{ number_format($leave['perm_credited'], 0) }}</div>
                        <div class="mini-stat-lbl">Credited</div>
                    </div>
                    <div class="mini-stat">
                        <div class="mini-stat-val val-used">{{ number_format($leave['perm_used'], 0) }}</div>
                        <div class="mini-stat-lbl">Used</div>
                    </div>
                    <div class="mini-stat">
                        <div class="mini-stat-val val-balance">{{ number_format($leave['perm_balance'], 0) }}</div>
                        <div class="mini-stat-lbl">Balance</div>
                    </div>
                </div>
        ...
        <div class="leave-block">
            ...
            <div class="leave-block-body">
                <div class="mini-stat-row">
                    <div class="mini-stat">
                        <div class="mini-stat-val val-credited">{{ number_format($leave['sat_credited'], 0) }}</div>
                        <div class="mini-stat-lbl">Credited</div>
                    </div>
                    <div class="mini-stat">
                        <div class="mini-stat-val val-used">{{ number_format($leave['sat_used'], 0) }}</div>
                        <div class="mini-stat-lbl">Used</div>
                    </div>
                    <div class="mini-stat">
                        <div class="mini-stat-val val-balance">{{ number_format($leave['sat_balance'], 0) }}</div>
                        <div class="mini-stat-lbl">Balance</div>
                    </div>
                </div>
...
(function () {
    ...
    var CAL_DATA_URL = '{{ route("employee.calendar-data") }}';
    ...
    function loadCalendar(year, month) {
        ...
        .then(function (d) {
            ...
            // Rebuild grid (keep day headers, replace day cells)
